...
```json
{
  ...
  "error": {
    ...
    "suggestions": ["等待 60 秒后重试", "考虑使用批量查询"]
  },
  "retry_after": 60
}
```
...
1. **实现指数退避**: 收到 429 后，等待时间翻倍重试
2. **缓存响应**: 对相同查询缓存响应，减少请求次数
3. **批量查询**: 使用 WebSocket 订阅代替轮询
4. **监控限流头**: 在 `X-RateLimit-Remaining` 接近 0 时主动降速
...
| 方法 | 端点 | 说明 |
|------|------|------|
| POST | `/api/v1/voice-query` | 语音查询 (增强版) |
| GET | `/api/v1/health` | 详细健康检查 |
| GET | `/api/v1/health/summary` | 快速健康摘要 |

### Kubernetes 健康探针

| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/api/health/live` | 存活探针 (Liveness) |
